...
struct Smile: Shape {
    ...
    func path(in rect: CGRect) -> Path {
        return Path{ path in
            ...
            let downRadius: CGFloat = 25 * (value)
            path.move(to: CGPoint(x: center - 150, y: 0))
            
            let to1 = CGPoint(x: center, y: downRadius)
            let control1 = CGPoint(x: center - 150, y: 0)
            let control2 = CGPoint(x: center - 150, y: downRadius)
            
            let to2 = CGPoint(x: center + 150, y: 0)
            let control3 = CGPoint(x: center + 150, y: downRadius)
            let control4 = CGPoint(x: center + 150, y: 0)
